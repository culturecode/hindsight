module Hindsight
  module Associations
    module ClassMethods
      ...
      def association_versioning_status
        all = reflections.keys.select {|association| copyable_association?(association) }
        Hash.new.tap do |status|
          status[:versioned]              = versioned_associations
          status[:unversioned_history]    = all - status[:versioned]
          status[:unversioned_no_history] = reflections.select {|association, reflection| status[:unversioned_history].include?(association) && reflection.is_a?(ActiveRecord::Reflection::HasManyReflection) }.keys
          status[:unversioned_history]    = status[:unversioned_history] - status[:unversioned_no_history]
          status[:ignored]                = ignored_associations
          status[:through]                = through_associations
        end
      end
      ...
      def versionable_reflection?(association)
        reflect_on_association(association).is_a?(ActiveRecord::Reflection::HasManyReflection)
      end
    ...
    module InstanceMethods
      ...
      def copy_associations_to(new_version)
        self.class.reflections.each do |association, reflection|
          ...
          next if self.class.send :version_association?, association
          next if !self.class.send :versionable_reflection?, association

          records = send(association).to_a

          if self.class.send :versioned_association?, association
            records.collect! {|r| r.send(:build_new_version) }
          elsif self.class.send :through_association?, association
            # duplicate the record so other metadata is retained
            records.collect! {|r| r.dup }
          end

          # assign the dups to the new_version's association
